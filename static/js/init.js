const ipcRenderer = window._SeablueConfig._ipcRenderer;
ipcRenderer.on("show", (e) => {
    $('.ComeMylife-title').css('-webkit-app-region',"drag");
    $(':root').css('animation',"show 0.2s linear forwards");
});

// 对应下面的 win.webContents.send("hide", s);
ipcRenderer.on("hide", (e, s) => {
    $('.ComeMylife-title').css('-webkit-app-region',"drag");
    $(':root').css('animation',`hide ${s}s linear forwards`);
});
...
new Vue({
    ...
    methods: {
        outline() {
            // 最小化通信
            window._SeablueConfig._ipcRenderer.send('xSystem', {code:0});
        },
        circle() {
            // 托盘化通信
            window._SeablueConfig._ipcRenderer.send('tSystem', {code:0});
        },
        // 弹出层示例
        dac() {
    ...
    */
        handSuccessFolder(e,f,l) {
           ...
           let a = f.raw.path.substring(0,(f.raw.path.lastIndexOf("\\")))
           a = a.split('\\');
           a = a.pop();
            const id = parseInt(moment().format('YYYYMMDDhmmss'));
            let arr = {
    ...
    */
        ...
        _OpenEXE(dataUrl) {
            ...
            shell.openItem(dataUrl);
            /* const qqLib = 'let a = 'D:\\Tools\\install\\Tencent\\QQ\\Bin\\QQ.exe;
    ...
    created() {
        const _fs = window._SeablueConfig._fs;
        // 创建写入流 onCreateStatus
        /* this._ipcRenderer.send('setData', {arr:obj.data,data:data}); */
        this._initMain(_fs,this._dataUrl);
